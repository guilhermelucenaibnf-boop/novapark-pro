...
def inicializar_banco():
    ...
    tabelas_colunas = {
        ...
        'configuracoes': [
            ...
            ('valor_tabela_1h','REAL DEFAULT 10'),('valor_tabela_2h','REAL DEFAULT 20'),('valor_tabela_3h','REAL DEFAULT 30'),('valor_tabela_4h','REAL DEFAULT 40'),
            ('taxa_talao','REAL DEFAULT 30'),('total_vagas','INTEGER DEFAULT 50'),('impressora_largura','INTEGER DEFAULT 58'),('logo','TEXT')
        ],
...
def calcular_cobranca(v, cfg, minutos, talao_perdido=False, desconto=0):
    tipo = v['tipo_tarifa']
    diaria = float(cfg['valor_diaria'] or 0)
    if minutos <= 15 or tipo == 'mensalista':
        bruto = 0.0
    elif tipo == 'hora':
        bruto = float(cfg['valor_hora'] or 0)
        if minutos > 60:
            fracoes = math.ceil((minutos - 60) / max(1, int(cfg['minutos_fracao'] or 30)))
            bruto += fracoes * float(cfg['valor_fracao'] or 0)
        if diaria > 0:
            bruto = min(bruto, diaria)
    elif tipo == 'hora_tabela':
        if minutos <= 60:
            bruto = float(cfg['valor_tabela_1h'] or 0)
        elif minutos <= 120:
            bruto = float(cfg['valor_tabela_2h'] or 0)
        elif minutos <= 180:
            bruto = float(cfg['valor_tabela_3h'] or 0)
        elif minutos <= 240:
            bruto = float(cfg['valor_tabela_4h'] or 0)
        else:
            bruto = diaria
        if diaria > 0:
            bruto = min(bruto, diaria)
    ...
    if talao_perdido:
        bruto += float(cfg['taxa_talao'] or 0)
    ...
    <div class="modal fade" id="mEntrada" tabindex="-1"><div class="modal-dialog"><div class="modal-content"><div class="modal-body">
        ...
        <form action="/entrada" method="POST">
            ...
            <select name="tipo_tarifa" id="tipoTarifa" class="form-select mb-2" onchange="atualizarTarifa()" required>
                ...
                <option value="hora_tabela" data-valor="{{ cfg.valor_tabela_1h }}">Tabela por horas (1h/2h/3h/4h) — a partir de R$ {{ "%.2f"|format(cfg.valor_tabela_1h) }}</option>
    ...
    <div class="modal fade" id="mConfig" tabindex="-1"><div class="modal-dialog"><div class="modal-content"><div class="modal-body">
        <form action="/salvar_config" method="POST" enctype="multipart/form-data">
            ...
            <div class="border rounded p-2 mb-2">
                <div class="small fw-bold text-primary mb-1">Tabela por Horas</div>
                <div class="row">
                    <div class="col-6"><label class="small fw-bold">1 hora:</label><input name="valor_tabela_1h" type="number" step="0.01" value="{{ cfg.valor_tabela_1h }}" class="form-control mb-2"></div>
                    <div class="col-6"><label class="small fw-bold">2 horas:</label><input name="valor_tabela_2h" type="number" step="0.01" value="{{ cfg.valor_tabela_2h }}" class="form-control mb-2"></div>
                    <div class="col-6"><label class="small fw-bold">3 horas:</label><input name="valor_tabela_3h" type="number" step="0.01" value="{{ cfg.valor_tabela_3h }}" class="form-control mb-2"></div>
                    <div class="col-6"><label class="small fw-bold">4 horas:</label><input name="valor_tabela_4h" type="number" step="0.01" value="{{ cfg.valor_tabela_4h }}" class="form-control mb-2"></div>
                </div>
                <div class="form-text">Acima de 4 horas, ou ao atingir o valor da diária, será cobrado no máximo o valor da diária.</div>
            </div>
            ...
            <select name="imp" class="form-select mb-2" required>
                {% set imp_atual = cfg.impressora_status or 'Automática / Genérica ESC/POS' %}
                {% for imp in ['Automática / Genérica ESC/POS','KA-1445','Thermer Bluetooth','Elgin','Bematech','Epson','Knup','Goldensky','Goojprt','Rongta','Xprinter','Zjiang','Datec','Exbom','MPT-II / MPT-III','Sweda','Tanca','Zebra','Bixolon','Star Micronics','Sewoo','Outra ESC/POS'] %}
                <option value="{{ imp }}" {% if imp_atual == imp %}selected{% endif %}>{{ imp }}</option>
                {% endfor %}
            </select>
            <label class="small fw-bold">Largura do papel:</label>
            <select name="impressora_largura" class="form-select mb-3">
                <option value="58" {% if (cfg.impressora_largura or 58)|int == 58 %}selected{% endif %}>58 mm — portátil</option>
                <option value="80" {% if (cfg.impressora_largura or 58)|int == 80 %}selected{% endif %}>80 mm</option>
            </select>
            <div class="form-text mb-3">Use “Automática / Genérica ESC/POS” para impressoras compatíveis que não aparecem na lista.</div>
...
def salvar_config():
    ...
    try:
        ...
        conn.execute("UPDATE configuracoes SET nome=?, cnpj=?, endereco=?, telefone=?, horario=?, mensagem=?, impressora_status=?, valor_diaria=?, valor_van=?, valor_pernoite=?, valor_hora=?, valor_fracao=?, minutos_fracao=?, valor_tabela_1h=?, valor_tabela_2h=?, valor_tabela_3h=?, valor_tabela_4h=?, taxa_talao=?, total_vagas=?, impressora_largura=?, logo=? WHERE empresa_id=?",
                     (
                         nome,
                         ...
                         max(1, int(request.form.get('minutos_fracao', 30))),
                         float(request.form.get('valor_tabela_1h', 10)), float(request.form.get('valor_tabela_2h', 20)),
                         float(request.form.get('valor_tabela_3h', 30)), float(request.form.get('valor_tabela_4h', 40)),
                         float(request.form.get('taxa_talao', 30)), max(1, int(request.form.get('total_vagas', 50))),
                         80 if request.form.get('impressora_largura') == '80' else 58,
                         logo, session['empresa_id']
                     ))
        conn.commit()
        conn.close()
    except Exception as e:
...
function salvarConfig(){if(cfg.perfil!=='admin')return;let n={perfil:'admin',nome:$('cNome').value.trim()||'GLPPARK',cnpj:$('cCnpj').value,endereco:$('cEndereco').value,telefone:$('cTelefone').value,horario:$('cHorario').value,diaria:Number($('cDiaria').value),van:Number($('cVan').value),pernoite:Number($('cPernoite').value),hora:Number($('cHora').value),fracao:Number($('cFracao').value),minutos_fracao:Math.max(1,Number($('cMinutosFracao').value)||30),tabela_1h:Number($('cTabela1h').value),tabela_2h:Number($('cTabela2h').value),tabela_3h:Number($('cTabela3h').value),tabela_4h:Number($('cTabela4h').value),taxa_talao:Number($('cTaxaTalao').value),total_vagas:Math.max(1,Number($('cTotalVagas').value)||50),mensagem:$('cMensagem').value,impressora:$('cImpressora').value,impressora_largura:Number($('cImpressoraLargura').value)||58,logo:cfg.logo||''},f=$('cLogo').files[0],fim=()=>{cfg=n;localStorage.setItem(KC,JSON.stringify(cfg));fila.push({acao:'config',dados:cfg});persistir();aplicar();alert('Configurações salvas no celular.')};if(f){if(f.size>2097152)return alert('A logo deve ter no máximo 2 MB.');let r=new FileReader();r.onload=()=>{n.logo=r.result;fim()};r.readAsDataURL(f)}else fim()}
async function sincronizar(){if(!navigator.onLine)return;if(!fila.length){$('rede').textContent='Online';setTimeout(()=>location.replace('/dashboard'),300);return}try{let r=await fetch('/api/sincronizar',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({empresa_chave:empresa,operacoes:fila})});if(r.ok){fila=[];persistir();$('rede').textContent='Sincronização concluída';setTimeout(()=>location.replace('/dashboard'),700)}else if(r.status===401){$('rede').textContent='Sessão encerrada — faça login novamente';setTimeout(()=>location.replace('/'),900)}else if(r.status===403){$('rede').textContent='Dados offline pertencem a outra empresa — preservados no celular'}else{let d={};try{d=await r.json()}catch(_e){}$('rede').textContent='Falha ao sincronizar — dados preservados'+(d.erro?' ('+d.erro+')':'')}}catch(e){$('rede').textContent='Sem internet — dados seguros no celular'}}function rede(){$('rede').textContent=navigator.onLine?'Online — sincronizando...':'Modo offline — dados seguros no celular';if(navigator.onLine)sincronizar()}window.addEventListener('online',rede);window.addEventListener('offline',rede);aplicar();listar();rede();if('serviceWorker'in navigator)navigator.serviceWorker.register('/sw.js');
</script></body></html>'''
    ...
    try:
        for op in corpo.get('operacoes',[]):
            ...
            if a=='entrada' and d.get('offline_id'):
                ...
                conn.execute('''UPDATE configuracoes SET nome=?,cnpj=?,endereco=?,telefone=?,horario=?,mensagem=?,impressora_status=?,valor_diaria=?,valor_van=?,valor_pernoite=?,valor_hora=?,valor_fracao=?,minutos_fracao=?,valor_tabela_1h=?,valor_tabela_2h=?,valor_tabela_3h=?,valor_tabela_4h=?,taxa_talao=?,total_vagas=?,impressora_largura=?,logo=? WHERE empresa_id=?''',(d.get('nome','GLPPARK'),d.get('cnpj',''),d.get('endereco',''),d.get('telefone',''),d.get('horario',''),d.get('mensagem',''),d.get('impressora',''),float(d.get('diaria',50)),float(d.get('van',30)),float(d.get('pernoite',40)),float(d.get('hora',10)),float(d.get('fracao',5)),max(1,int(d.get('minutos_fracao',30))),float(d.get('tabela_1h',10)),float(d.get('tabela_2h',20)),float(d.get('tabela_3h',30)),float(d.get('tabela_4h',40)),float(d.get('taxa_talao',30)),max(1,int(d.get('total_vagas',50))),80 if int(d.get('impressora_largura',58))==80 else 58,d.get('logo',''),eid))
        conn.commit()
