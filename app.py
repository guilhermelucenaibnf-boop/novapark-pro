    ...
    <div class="modal fade show" id="modalEntradaSucesso" tabindex="-1" style="display: block; background: rgba(0,0,0,0.6);" aria-modal="true" role="dialog">
        <div class="modal-dialog modal-dialog-centered"><div class="modal-content"><div class="modal-body text-center bg-white p-4">
            ...
            <div id="printableArea">
                {% if cfg.logo %}<img src="/logo?v={{ cfg.id }}" alt="Logo" style="width:28px; height:28px; object-fit:contain; display:block; margin:0 auto 4px auto;"><br>{% endif %}
    ...
    try:
        for op in corpo.get('operacoes',[]):
            ...
            if a=='entrada' and d.get('offline_id'):
                ...
                conn.execute('''UPDATE configuracoes SET nome=?,cnpj=?,endereco=?,telefone=?,horario=?,mensagem=?,impressora_status=?,valor_diaria=?,valor_van=?,valor_pernoite=?,valor_hora=?,valor_fracao=?,minutos_fracao=?,taxa_talao=?,total_vagas=?,logo=? WHERE empresa_id=?''',(d.get('nome','GLPPARK'),d.get('cnpj',''),d.get('endereco',''),d.get('telefone',''),d.get('horario',''),d.get('mensagem',''),d.get('impressora',''),float(d.get('diaria',50)),float(d.get('van',30)),float(d.get('pernoite',40)),float(d.get('hora',10)),float(d.get('fracao',5)),max(1,int(d.get('minutos_fracao',30))),float(d.get('taxa_talao',30)),max(1,int(d.get('total_vagas',50))),d.get('logo',''),eid))
        conn.commit()
